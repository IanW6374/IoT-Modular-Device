...
PORTAL_CSS = (
    ...
    'background:rgba(8,126,139,.09);color:var(--accent2);text-decoration:none}'
    '.compact,button.compact{padding:7px 10px;font-size:.82rem}.danger,button.danger{'
    'background:var(--bad);border-color:var(--bad);color:#fff}.danger:hover,button.danger:hover{'
    'background:#8f2828;border-color:#8f2828;color:#fff}button:disabled{cursor:not-allowed;'
    'border-color:#d3dde1;background:#e7ecee;color:#7b8a91;opacity:1}'
    'button:disabled:hover{border-color:#d3dde1;background:#e7ecee;color:#7b8a91}'
    ...
    '.api-scope-editor{margin-top:14px}.api-scope-editor form{margin-top:10px}'
    '.api-scope-select{min-height:10.5rem}'
    ...
    'text-align:center}.workflow-steps li:not(:last-child):after{content:"";position:absolute;z-index:0;'
    'left:calc(50% + 18px);right:calc(-50% + 18px);top:17px;height:2px;background:var(--line)}'
    '.workflow-step-number{position:relative;z-index:1;display:grid;place-items:center;width:35px;height:35px;'
    'border:2px solid var(--line);border-radius:50%;background:var(--surface);color:var(--accent)}'
    '.qualification-action-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(20rem,1fr));'
    'gap:16px;margin:20px 0}.qualification-action{display:flex;flex-direction:column;padding:18px;'
    'border:1px solid var(--line);border-radius:14px;background:var(--soft)}'
    '.qualification-action h3{margin-bottom:5px}.qualification-action form{display:flex;flex:1;'
    'flex-direction:column}.qualification-action form>.actions{margin-top:auto;padding-top:14px}'
    ...
    '.metric-link{color:inherit;text-decoration:none;cursor:pointer;transition:border-color .15s ease,box-shadow .15s ease,'
    ...
    '.upgrade-source-divider:before,.upgrade-source-divider:after{content:"";height:1px;flex:1;background:var(--line)}'
    '.upgrade-settings{margin:0}.upgrade-settings>summary{display:flex;align-items:center;gap:16px;cursor:pointer;'
    'list-style:none}.upgrade-settings>summary::-webkit-details-marker{display:none}.upgrade-settings>summary span{'
    'display:grid;gap:3px;flex:1;min-width:0}.upgrade-settings>summary small{display:block;color:var(--muted);'
    'font-weight:500}.upgrade-settings>summary:after{content:"Show";margin-left:auto;color:var(--accent);'
    'font-size:.78rem;font-weight:750;flex:0 0 auto}.upgrade-settings[open]>summary:after{content:"Hide"}'
    '.upgrade-settings-body{margin-top:18px;padding-top:18px;border-top:1px solid var(--line)}'
    ...
    '.manual-upgrade-workspace{display:grid;grid-template-columns:minmax(0,1fr);'
    'gap:20px;align-items:stretch}.upgrade-steps-panel{padding:0;min-width:0}'
    '.upgrade-operation{min-width:0;display:flex;flex-direction:column}.upgrade-operation>form{display:flex;'
    'flex-direction:column;min-height:0}.upgrade-operation .actions{margin-top:auto;padding-top:14px}'
    '.upgrade-operation>#upgrade-check-form{flex-direction:row;flex-wrap:wrap;align-items:center;gap:12px;'
    'margin:0 0 16px}#upgrade-check-form p{margin:0}#upgrade-check-form .actions{margin:0 0 0 auto;padding:0}'
    '.staged-upgrade-copy{max-width:44rem}.staged-upgrade-copy h3{margin:12px 0 5px}'
    '.staged-upgrade-copy p{margin:0;color:var(--muted)}'
    '.upgrade-stage-list{display:flex;align-items:stretch;min-width:0;'
    'gap:0;margin:0;padding:0;list-style:none;font-size:.82rem}'
    '.upgrade-stage-list li{--step-progress:0%;--step-colour:var(--muted);position:relative;display:grid;'
    'flex:1 1 0;grid-template-rows:4.25rem 3.25rem auto;justify-items:center;align-items:end;min-width:0;'
    'color:var(--muted);text-align:center}.upgrade-stage-list li:not(:last-child):after{content:"";position:absolute;'
    'z-index:0;left:50%;right:-50%;top:5.8rem;height:3px;background:var(--line)}'
    '.upgrade-stage-name{display:flex;align-items:flex-end;justify-content:center;min-width:0;max-width:9rem;'
    'padding:0 5px 8px;line-height:1.2;overflow-wrap:anywhere}.upgrade-stage-ring{position:relative;z-index:1;display:grid;'
    'place-items:center;width:3.25rem;height:3.25rem;border-radius:50%;background:conic-gradient('
    'var(--step-colour) var(--step-progress),var(--line) 0);font-size:.72rem;font-weight:800;'
    'font-variant-numeric:tabular-nums}.upgrade-stage-ring:before{content:"";position:absolute;inset:5px;'
    'border-radius:50%;background:var(--surface)}.upgrade-stage-percent{position:relative;z-index:1}'
    '.upgrade-stage-list li.active{--step-colour:var(--accent);color:var(--ink);font-weight:700}'
    '.upgrade-stage-list li.active .upgrade-stage-ring{outline:2px solid var(--accent);outline-offset:2px}'
    ...
    '.upgrade-version-selection{display:grid;justify-items:center;gap:7px;width:100%}.upgrade-file-selection{margin:0}'
    '.upgrade-stage-step-control select{min-width:0;width:100%}.upgrade-version-selection select{border:2px solid '
    'var(--accent);background:#f3fbfc;color:var(--ink);font-weight:800;text-align:center}'
    '.upgrade-version-selection>form{display:grid;justify-items:center;gap:7px;width:100%;margin:0}'
    '.upgrade-version-selection #upgrade-check-form .actions{justify-content:center;margin:0;padding:0}'
    ...
    '.selected-release-summary{display:grid;justify-items:center;gap:5px;width:100%;padding:10px;border:2px solid '
    'var(--accent);border-radius:10px;background:#f3fbfc;color:var(--ink)}.selected-release-summary>span{color:var(--accent2);'
    'font-size:.66rem;font-weight:800;letter-spacing:.06em;text-transform:uppercase}.selected-release-summary strong{'
    'font-size:.8rem;overflow-wrap:anywhere}.selected-release-summary small{color:var(--muted);font-size:.68rem;line-height:1.25}'
    '.upgrade-method-choices{display:grid;'
    'grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;'
    'margin:10px 0 20px}.upgrade-method-choice{display:grid;gap:5px;align-content:start;min-height:5rem;padding:14px;'
    'border:1px solid var(--line);border-radius:11px;background:var(--soft);color:var(--ink);text-decoration:none}'
    '.upgrade-method-choice:hover,.upgrade-method-choice:focus,.upgrade-method-choice:focus-visible{'
    'border-color:var(--accent);outline:2px solid var(--accent);outline-offset:2px;text-decoration:none}'
    '.upgrade-method-choice strong{color:var(--accent)}.upgrade-method-choice small{'
    'color:var(--muted);font-weight:500;line-height:1.4}.upgrade-method-choice[aria-disabled="true"]{'
    'opacity:.55;pointer-events:none}.upgrade-method-choice[aria-current="true"]{border-color:var(--accent);'
    'box-shadow:inset 0 0 0 1px var(--accent)}.change-upgrade-method{margin-right:auto}'
    '.upgrade-summary-details>summary{cursor:pointer;font-weight:700}.upgrade-summary-details[open]>summary{margin-bottom:16px}'
    '.manual-upgrade-buttons{justify-content:flex-end}.manual-upgrade-buttons form{margin:0}'
    '.upgrade-method-choice{grid-template-columns:minmax(0,1fr) auto}'
    '.upgrade-method-choice>strong{grid-column:1;grid-row:1}.upgrade-method-choice>small{grid-column:1/-1}'
    '.upgrade-method-choice>.badge{grid-column:2;grid-row:1;justify-self:end;align-self:start}'
    '.qualification-retry{margin-top:12px;text-align:left}'
    '.qualification-retry summary{cursor:pointer;font-weight:600}.qualification-retry .field{margin-top:12px}'
    '.qualification-gates{grid-template-columns:repeat(auto-fit,minmax(min(100%,260px),1fr))}'
    ...
    'white-space:nowrap;border:0}.file-button{cursor:pointer}.file-name,.file-guidance{color:var(--muted);'
    'overflow-wrap:anywhere}.file-name+.file-guidance{margin-left:1.25rem}.code-editor{min-height:30rem;'
    ...
    '.settings-save-bar{background:rgba(24,37,43,.97)}}'
    ...
    '.release-available{align-items:stretch;flex-direction:column}.release-available form{margin-left:0}'
...
PORTAL_JS = (
    ...
    'expanded=!subgroup.classList.contains("open");subgroup.classList.toggle("open",expanded);'
    'trigger.setAttribute("aria-expanded",expanded?"true":"false");return;}var group=trigger.parentNode,'
    'open=!group.classList.contains("open");close(group);group.classList.toggle("open",open);'
    'trigger.setAttribute("aria-expanded",open?"true":"false");});document.addEventListener("keydown",function(e){var active='
    ...
    'var restart=document.getElementById("restart-required");function renderRestartStatus(s){if(!restart||!s)return;'
    'restart.hidden=!s.required;var label=document.getElementById("restart-required-label");if(label&&s.required)'
    'label.textContent=s.reason_count===1?"Restart required":"Restart required ("+(s.reason_count||0)+" changes)";}'
    'window.portalRefreshRestart=function(state){if(state){renderRestartStatus(state);return Promise.resolve(state);}'
    'return fetch("/api/restart-required",{cache:"no-store",credentials:"same-origin"}).then(function(r){return r.ok?'
    'r.json():null;}).then(function(s){renderRestartStatus(s);return s;}).catch(function(){return null;});};'
    'if(restart)window.portalRefreshRestart();'
    'function portalFormStatus(form){var out=form.querySelector("[data-portal-form-status]");if(out)return out;'
    'out=document.createElement("span");out.setAttribute("data-portal-form-status","");out.className='
    '"portal-status action-form-status";var actions=form.querySelector(".actions");if(actions)actions.insertBefore(out,'
    'actions.firstChild);else form.appendChild(out);return out;}'
    'function portalSavedDefaults(form){Array.from(form.elements||[]).forEach(function(field){if(!field.name)return;'
    'if(field.type==="password"){field.value="";field.defaultValue="";}else if(field.type==="checkbox"||field.type==='
    '"radio")field.defaultChecked=field.checked;else if(field.tagName==="SELECT")Array.from(field.options).forEach('
    'function(option){option.defaultSelected=option.selected;});else if(field.type!=="file")field.defaultValue=field.value;});}'
    'window.portalRefreshTarget=function(selector,url,message){return fetch(url||location.pathname+location.search,'
    '{cache:"no-store",credentials:"same-origin"}).then(function(r){if(r.status===401){location.replace('
    '"/login?reason=expired");throw new Error("Session expired");}if(!r.ok)throw new Error("Unable to refresh page section");'
    'return r.text();}).then(function(html){var parsed=new DOMParser().parseFromString(html,"text/html"),fresh='
    'parsed.querySelector(selector),current=document.querySelector(selector);if(!fresh||!current)throw new Error('
    '"Updated page section is unavailable");current.replaceWith(fresh);document.dispatchEvent(new CustomEvent('
    '"portal:content-updated",{detail:{selector:selector}}));if(message){var out=document.createElement("p");'
    'fresh.insertBefore(out,fresh.firstChild);portalStatus(out,"success",message);}return fresh;});};'
    'document.addEventListener("submit",function(event){var form=event.target;if(!form.matches||!form.matches('
    '"form[data-portal-async]"))return;if(event.defaultPrevented||!form.reportValidity())return;event.preventDefault();var button='
    'event.submitter||form.querySelector("button[type=submit],input[type=submit]"),original=button?button.textContent:"",'
    'out=portalFormStatus(form),params=new URLSearchParams();new FormData(form).forEach(function(value,key){'
    'params.append(key,value);});if(event.submitter&&event.submitter.name)params.append(event.submitter.name,'
    'event.submitter.value);if(button){button.disabled=true;if(button.dataset.busyLabel)button.textContent='
    'button.dataset.busyLabel;}portalStatus(out,"",form.dataset.portalStatus||"Saving changes…");fetch(form.action,{method:"POST",credentials:'
    '"same-origin",headers:{"Accept":"application/json","Content-Type":"application/x-www-form-urlencoded"},'
    'body:params.toString()}).then(function(response){return response.text().then(function(body){var payload={};'
    'try{payload=body?JSON.parse(body):{};}catch(error){payload={message:body};}if(response.status===401){location.replace('
    '"/login?reason=expired");throw new Error("Session expired");}if(!response.ok)throw new Error(payload.error||'
    'payload.message||"The change could not be saved");return payload;});}).then(function(payload){form.dataset.portalDirty='
    '"0";portalSavedDefaults(form);if(payload.reset_form)form.reset();if(payload.restart)portalRefreshRestart(payload.restart);'
    'else portalRefreshRestart();if(payload.task_id&&typeof refreshTasks==="function")refreshTasks();var refreshTarget='
    'payload.refresh_target||form.dataset.portalRefreshTarget,refreshUrl=payload.refresh_url||form.dataset.portalRefreshUrl;'
    'if(refreshTarget)return portalRefreshTarget(refreshTarget,refreshUrl,payload.message);portalStatus(out,"success",'
    'payload.message||"Changes saved");if(payload.redirect)location.assign(payload.redirect);}).catch(function(error){if('
    'error.message!=="Session expired")portalStatus(out,"error",error.message);}).finally(function(){if(button&&document.contains('
    'button)){button.disabled=false;if(button.dataset.busyLabel)button.textContent=original;}});});'
    ...
    'function markDirty(event){var form=event.target&&event.target.form;if(form&&form.hasAttribute("data-portal-dirty"))'
    'form.dataset.portalDirty="1";}document.addEventListener("input",markDirty,true);document.addEventListener("change",'
    'markDirty,true);document.addEventListener("reset",function(event){if(!event.target.matches("form[data-portal-dirty]"))'
    'return;event.target.dataset.portalDirty="0";var out=event.target.querySelector("[data-portal-form-status]");'
    'if(out)portalStatus(out,"","");setTimeout(function(){document.dispatchEvent(new CustomEvent("portal:form-reset",'
    '{detail:{form:event.target}}));},0);},true);window.addEventListener("beforeunload",function(e){var dirty='
    'document.querySelector("form[data-portal-dirty][data-portal-dirty=\\\"1\\\"]");if(!dirty)return;e.preventDefault();'
    'e.returnValue="";});'
...
def save_actions(label='Save changes', discard=True):
    """Render consistent in-place save controls for configuration forms."""
    discard_button = (
        '<button class="secondary" type="reset">Discard changes</button>'
        if discard else ''
    )
    return (
        '<div class="actions settings-save-bar">'
        '<span class="portal-status action-form-status" data-portal-form-status '
        'role="status" aria-live="polite"></span>'
        '<div class="settings-save-controls">' + discard_button +
        '<button type="submit" data-busy-label="Saving…">' +
        escape(label) + '</button></div></div>'
    )
